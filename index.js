...
function removeFlavorByName(arr, flavor) {
  ...
  const target = arr.indexOf(flavor);

  // using slice()
  // we don't need to worry about boundaries here, we get empty array at the boundary
  // return arr.slice(0,target).concat(arr.slice(target+1));
...
function getAverageWordLength(arr) {
  //this is doable in one line, but it requires higher order function.
  //return arr.map(i => i.split(" ")).reduce((a,b) => a+b.length,0)/arr.length
  let total = 0;
  for (let i = 0; i < arr.length; i++) {
...
//const ensemble = originalFlavors.concat(newFlavors).concat(seasonalFlavors).concat(regionalFlavors)

function getRandomFlavors(
  arr1 = originalFlavors,
  arr2 = newFlavors,
  arr3 = seasonalFlavors,
  arr4 = regionalFlavors
) {
  const ensemble = arr1.concat(arr2).concat(arr3).concat(arr4);
  const randomFlavors = [];
  for (let i = 0; i < 31; i++) {
    // we probably don't want duplicates so we are going to implement accordingly;
    const currentPickedIndex = Math.floor(Math.random() * ensemble.length);
    randomFlavors.push(ensemble[currentPickedIndex]);
    // We can do a trick like this to quick delete a element from an array when perserving order is not required, like in this case of randomly picking items.
    ensemble[currentPickedIndex] = ensemble.pop();
  }
